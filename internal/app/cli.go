...
func (c *cli) parse() {
	...
	flag.BoolVar(&c.verbose, "verbose", true, "show verbose execution logs.")
	...
	flag.BoolVar(&c.noNs, "no-ns", true, "don't create namespaces")
	flag.BoolVar(&c.skipValidation, "skip-validation", false, "skip desired state validation")
	flag.BoolVar(&c.keepUntrackedReleases, "keep-untracked-releases", true, "keep releases that are managed by Helmsman from the used DSFs in the command, and are no longer tracked in your desired state.")
